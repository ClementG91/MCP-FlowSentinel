...
GO_PATCH="${GO_PATCH:-0}"
if [[ $GO_MAJOR -lt 1 || ($GO_MAJOR -eq 1 && ($GO_MINOR -lt 27 || ($GO_MINOR -eq 27 && $GO_PATCH -lt 1))) ]]; then
    fail "Go $GO_VERSION found, but 1.27.1+ required. Update: https://go.dev/dl/"
fi
ok "$(go version)"
