...
- CodeQL now runs from a pinned workflow (`.github/workflows/codeql.yml`, `security-extended` queries for Go and GitHub Actions) on every pull request, including Dependabot updates that GitHub's default setup skipped, plus pushes to `main` and a weekly schedule.
