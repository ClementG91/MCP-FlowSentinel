...
check-deps:
	@echo "==> Checking Go version (need ≥ 1.27.1)..."
	@go env GOVERSION | awk -F. '{major=$$1; sub(/^go/, "", major); patch=$$3+0; if (major < 1 || (major == 1 && ($$2 < 27 || ($$2 == 27 && patch < 1)))) {print "ERROR: Go 1.27.1+ required"; exit 1}}'
